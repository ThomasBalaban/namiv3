...
class InputFunnel:
        ...
        while not self.shutdown_requested:
            try:
                ...
                try:
                    ...
                    # response remains None, which will trigger the unlock logic in the handler
                
                # ALWAYS call handle_response, even if response is None
    ...
    def _handle_response(self, response, prompt_details, item):
        ...
        # Removed the 'if not response' check so the handler in main.py 
        # is always notified to release the nami_is_busy lock.
        
        if response:
            print(f"\nBot response via funnel: {response[:100]}...")
        else:
            print("\nNo response generated by bot (Funnel notifying handler to unlock)")

        # Use the global response handler (FunnelResponseHandler in main.py)
        if self.response_handler and not self.shutdown_requested:
            try:
                # This call is now guaranteed, allowing main.py to clear the lock
